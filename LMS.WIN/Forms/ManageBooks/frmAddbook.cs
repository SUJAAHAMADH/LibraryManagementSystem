    ...
    public partial class frmAddbook : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (txtbookName.Text != string.Empty && txtAuthor.Text != string.Empty && txtPublisher.Text != string.Empty && cmboxLanguage.Text != string.Empty)
            {
                ...
                {
                    MessageBox.Show("Please add book quantity.");
                    return;
                }
                ...
                if (MessageBox.Show("Do you want to save the Book ?", "Done ?", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    //Add bookbarcode details in list
                    ...
                    for (int i = 0; i < gridBookList.Rows.Count; i++)
                    {
                        ...
                        {
                            BookBarcodeID = -1,
                            Barcode = gridBookList.Rows[i].Cells["Barcode"].Value.ToString()
                        };
                    ...
                    Book bookobj = new Book
                    {
                        ...
                        TotalQuantity = Convert.ToInt32(txtBookCopies.Text),
                        LanguageID = Convert.ToInt32(cmboxLanguage.SelectedValue),
                        Description = richtextDescription.Text,
                        AuthorID = Convert.ToInt32(authorObj.AuthorID),
                        PublisherID = Convert.ToInt32(publisherObj.PublisherID),
                    ...
                    try
                    {
                        Book obj = BookBL.Save(bookobj);
                        MessageBox.Show("Book save successfully.", "Book", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void txtBookCopies_TextChanged(object sender, EventArgs e)
        {
            string isRestricted = string.Empty;
            string barcode = string.Empty;
            if (comboBoxtype.SelectedItem == null)
            {
                MessageBox.Show("Please select book type.");
                txtBookCopies.Text = "0";
                return;
            }
            else
            {
                isRestricted = comboBoxtype.SelectedItem.ToString();
            }

            if (!string.IsNullOrEmpty(txtBookCopies.Text))
            {
                int totalBookCopies = Convert.ToInt32(txtBookCopies.Text);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < gridBookList.Rows.Count; i++)
            {
                ...
                {
                    Barcode =string.Concat(prefix,gridBookList.Rows[i].Cells[1].Value.ToString(),prefix)
                };
